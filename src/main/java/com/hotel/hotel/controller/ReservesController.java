...
public class ReservesController {
    ...
    public ReservesController(ReservaRepository reservaRepository, ClientsRepository clientsRepository,
            HabitacioRepository habitacioRepository, UsuarisRepository usuariRepository,
            HotelRepository hotelRepository, FacturaRepository facturaRepository) {
    ...
    public String reservarhabitacio(Model model, @ModelAttribute Reserva reserva, BindingResult result,
            HttpSession session, @RequestParam("metode_pagament") String metode_pagament) {
        ...
        long diesEstada = ChronoUnit.DAYS.between(reserva.getData_inici().toLocalDate(),
                reserva.getData_fi().toLocalDate());
    ...
    public String HabitacionsFiltradesperdata(@PathVariable Long id, @ModelAttribute("reserva") Reserva reservaTemporal,
        ...
        List<Reserva> reservasEntredatas = reservaRepository.findReservasSolapadas(id, reservaTemporal.getData_inici(),
                reservaTemporal.getData_fi());
            ...
            if (reservaOpt.isPresent()) {
                ...
                if (diesFaltants <= 0) {
                    redirectAttributes.addFlashAttribute("errorReserva",
                            "La reserva no se puede eliminar: queda 1 día o menos.");
                    return "redirect:/client/reserves/llistat";
                }
    ...
    public String guardarModificacionReserva(
        ...
        if (reservaOriginalOptional.isPresent()) {
            ...
            List<Reserva> reservasSolapadas = reservaRepository.findReservasSolapadas(
                    habitacioOriginal.getHotel().getId(), reservaModificada.getData_inici(),
                    reservaModificada.getData_fi());

            boolean habitacionDisponible = reservasSolapadas.stream()
                    .noneMatch(reserva -> !reserva.getId().equals(reservaModificada.getId())
                            && reserva.getHabitacio().getId().equals(habitacioOriginal.getId()));
            ...
            long diesEstada = ChronoUnit.DAYS.between(reservaOriginal.getData_inici().toLocalDate(),
                    reservaOriginal.getData_fi().toLocalDate());
            ...
            redirectAttributes.addFlashAttribute("error",
                    "No s'ha trobat la reserva amb ID: " + reservaModificada.getId());
            return "redirect:/admin/reserves/llistat";
        }
    }
